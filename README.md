...
- 状態管理（DynamoDB）  
  各フィードの設定と最終処理時刻を DynamoDB テーブル（`StateTableName` 出力を参照）で管理します。キー: `feedUrl`。属性例: `webhookUrl`(必須)、`categoryTerm`、`titleIncludes`、`lastPublishedAt`。`lastPublishedAt` は要約＋送信 Lambda が Discord 送信成功後にのみ更新します。
